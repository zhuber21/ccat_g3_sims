...
timestream_length = 60.0   # simulating 60 second timestream
...
output_fname = save_dir+"sixty_second_576_chs_smaller_quanta_test.g3"

writer = core.G3Writer(filename=output_fname,append=True)
writer(make_obs_frame(frame_num=0,start=True,board_id=board_id))
for i in range(1, n_frames+1):
    writer(make_data_frame(i,board_id,n_chs=n_chs,quanta=0.01))
    time.sleep(agg_time)
writer(make_obs_frame(frame_num=n_frames+1,start=False,board_id=board_id))
